...
function SurfaceHeader({ number, title, description }: { number: string; title: string; description: string }) {
  return (
    <div>
      ...
      <p className="mt-1.5 text-xs leading-5 text-white/35">{description}</p>
    </div>
  );
}
...
function SectionHeading({ eyebrow, title, methods }: { eyebrow: string; title: string; methods: string }) {
  return (
    <div className="mb-5 flex items-end justify-between">
      <div><p className="text-[10px] font-black uppercase tracking-[.2em] text-emerald-400">{eyebrow}</p><h2 className="mt-2 text-3xl">{title}</h2></div>
      <span className="font-mono text-[10px] text-white/25">{methods}</span>
    </div>
  );
}
  ...
  return (
    <div className="min-w-0" data-reveal="right">
      ...
      <div className="grid border-l border-t border-white/10 md:grid-cols-3">
        <article className={surfaceClass} data-reveal>
        ...
        <article className={surfaceClass} data-reveal>
          ...
          <div onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addLog(TEST_METHODS[1], summarizeFiles(Array.from(event.dataTransfer.files))); }} className="mt-8 flex min-h-24 items-center justify-center rounded-xl border border-dashed border-[#eefb7a]/35 bg-[#eefb7a]/5 p-4 text-center text-xs font-bold uppercase tracking-wider text-[#eefb7a]">Drop files here</div>
        </article>
        <article className={surfaceClass} data-reveal>
          <SurfaceHeader number="03" title="Paste" description="Focus the target and paste copied image or file data." />
          <div contentEditable suppressContentEditableWarning tabIndex={0} onPaste={(event) => addLog(TEST_METHODS[2], summarizeFiles(Array.from(event.clipboardData.files)))} className="mt-8 min-h-24 rounded-xl border border-white/12 bg-black/20 p-4 text-xs leading-5 text-white/30 outline-none transition focus:border-[#eefb7a]/50 focus:text-white/60">Click here, then press Ctrl+V or Cmd+V.</div>
        </article>
      </div>
      ...
      <div className="grid border-l border-t border-white/10 md:grid-cols-2">
        <article className={surfaceClass} data-reveal>
          ...
          <p className="mt-4 truncate rounded-lg bg-black/20 px-3 py-2 font-mono text-[10px] text-white/30">handle: {fileHandleName ?? 'none selected'}</p>
        </article>
        <article className={surfaceClass} data-reveal>
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <label className="mt-6 flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-dashed border-white/15 bg-black/20 p-4 transition hover:border-[#eefb7a]/40">
              ...
              <span className="min-w-0"><strong className="block truncate text-[10px] text-white/70">{networkFile?.name ?? 'Choose an upload file'}</strong><small className="mt-1 block font-mono text-[10px] text-white/25">{networkFile ? formatBytes(networkFile.size) : 'No file selected'}</small></span>
              <span className="rounded-lg bg-white px-3 py-2 text-[10px] font-black uppercase text-[#101416]">Browse</span>
            </label>
          </div>
...
export function EventConsole({ logs, onClear }: { logs: TestLog[]; onClear: () => void }) {
  return (
    <section className="border-t border-white/10 bg-[#101416]/90" data-reveal>
      <div className="mx-auto w-full max-w-360 px-5 py-16 sm:px-8 lg:px-12">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#eefb7a]">Live output</p><h2 className="mt-2 text-3xl">Event console</h2></div><button type="button" onClick={onClear} className={secondaryButtonClass}>Clear console</button></div>
        <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-[#080a0b] font-mono">
          ...
          <div className="max-h-96 min-h-48 overflow-y-auto p-3 sm:p-4">
            {logs.length ? logs.map((log) => (
              <div key={log.id} className="grid gap-1 border-b border-white/5 px-2 py-3 text-[10px] sm:grid-cols-[72px_210px_1fr]">
            ...
            )) : <div className="flex min-h-40 items-center justify-center text-[10px] uppercase tracking-[.16em] text-white/20">Waiting for your first action…</div>}
          </div>
        </div>
      </div>
    </section>
